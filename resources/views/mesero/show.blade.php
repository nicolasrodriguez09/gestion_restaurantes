...
<div class="max-w-7xl mx-auto px-6 py-10 space-y-6">
    @php
        ...
        $estadoMesa = strtolower($mesa->estado->nombreEstado ?? '');
    @endphp
    ...
    <div class="rounded-3xl border border-slate-200 bg-white shadow-sm p-6 space-y-4">
        <div class="flex items-start justify-between gap-4 flex-wrap">
            <div>
                ...
                <p class="text-sm text-slate-500">Ubicacion: {{ $mesa->ubicacion ?? 'No definida' }}</p>
            </div>
            <div class="flex flex-wrap gap-2">
                <span class="inline-flex items-center gap-2 rounded-full px-3 py-1 bg-slate-100 text-xs font-semibold text-slate-700">
                    <span class="h-2 w-2 rounded-full {{ str_contains($estadoMesa, 'ocup') ? 'bg-rose-500' : 'bg-emerald-500' }}"></span>
                    Estado: {{ $mesa->estado->nombreEstado ?? 'N/A' }}
                </span>
                <span class="inline-flex items-center gap-2 rounded-full px-3 py-1 bg-slate-100 text-xs font-semibold text-slate-700">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                @else
                    <ul class="space-y-3">
                        @foreach($pedidos as $pedido)
                            ...
                            @endphp
                            <li class="rounded-2xl border border-slate-100 bg-slate-50 p-4 space-y-3">
        ...
        <div class="rounded-3xl border border-slate-200 bg-white shadow-sm p-5 space-y-3">
            <h2 class="text-lg font-semibold text-slate-900 mb-1">Resumen de mesa</h2>
            ...
            <div class="mt-3 space-y-2">
                @php $ultimoPedido = $pedidos->first(); $porcMesa = $ultimoPedido ? $progress($ultimoPedido) : 0; @endphp
                <p class="text-xs text-slate-500">Avance del ultimo pedido</p>
                <div class="h-2 rounded-full bg-slate-100 overflow-hidden">
                    <div class="h-full rounded-full {{ $porcMesa === 100 ? 'bg-emerald-500' : ($porcMesa === 50 ? 'bg-amber-500' : 'bg-slate-300') }}" style="width: {{ $porcMesa }}%"></div>
                </div>
                <p class="text-xs text-slate-500">Pedidos activos: {{ $pedidos->count() }}</p>
            </div>
        </div>
    </div>
</div>
